...
for my $engine ( sort (@{$engine_list}) ) {
  ...
  if ((lc $action eq 'detach') || (lc $action eq 'update')) {
    ...
    if ( $type eq 'oracle' ) {
      ...
      $jobno = $db->addSource($sourcename,$sourceinst,$sourceenv,$source_os_user,$dbuser,$password,$dsourcename,$group,$logsync,$stageenv,$stageinst,$stage_os_user, $backup_dir, $dumppwd, $mountbase,$files);
    }
    elsif ($type eq 'mssql') {
